...
public:
	...
	/// @brief LightManagerのシングルトンインスタンスを取得
	...
	/// @brief ライト名を指定してライトを削除
	/// @param name 削除するライト名
	/// @return 削除できた場合はtrue
	bool Destroy(const std::string& name);

	/// @brief 指定したシーンに属するライトをすべて削除
	...
	/// @brief JsonファイルからEditor管理対象のライトを読み込み
...
private:
	...
	/// @brief ライト構造体内のuseLightへ有効状態を反映
	/// @param handle 対象ライトのハンドル
	/// @param enabled 有効にする場合はtrue
	void ApplyEnabledToLight(LightHandle handle, bool enabled);

	/// @brief ハンドルに紐づくライト名を検索マップから削除
