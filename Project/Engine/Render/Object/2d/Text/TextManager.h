...
public:
	...
	/// @brief 全Textと共有リソースを解放
	void Finalize();

	/// @brief 全Textへスクリーンサイズを設定
	/// @param width スクリーン幅
	...
	/// @brief GPUが対象を使用していないことが保証された時点でTextを即時削除
	...
	/// @brief 現在SceneのTextを元の描画順の連続レイヤーバッチとして描画
	/// @param currentSceneType 現在のScene
	...
	/// @brief Editor管理TextをJSONへ変換
	/// @return Text一覧を含むJSON
	nlohmann::json ToJson() const;

	/// @brief JSONからEditor管理Textを復元
	/// @param json Text一覧を含むJSON
	void FromJson(const nlohmann::json& json);

	/// @brief JSONから指定シーン所属のEditor管理Textを復元
	/// @param json Text一覧を含むJSON
	/// @param sceneType 復元対象のシーン、SceneType::None所属のTextも復元
	void FromJson(const nlohmann::json& json, SceneType sceneType);

	/// @brief Editor履歴のSnapshotからEditor管理Text全体を復元
	/// @param json Text一覧を含むSnapshot
	void RestoreEditorSnapshot(const nlohmann::json& json);

	/// @brief Editor管理TextをJSONファイルへ保存
	/// @param filePath 保存先
	/// @return 保存に成功した場合はtrue
	bool SaveToFile(const std::filesystem::path& filePath) const;

	/// @brief 指定シーン所属のEditor管理TextをJSONファイルへ保存
	/// @param filePath 保存先のファイルパス
	/// @param sceneType 保存対象のシーン、SceneType::None所属のTextも保存
	/// @return 保存に成功した場合はtrue
	bool SaveToFile(const std::filesystem::path& filePath, SceneType sceneType) const;

	/// @brief JSONファイルからEditor管理Textを読み込み
	/// @param filePath 読み込み元
	/// @return 読み込みに成功した場合はtrue
	bool LoadFromFile(const std::filesystem::path& filePath);

	/// @brief JSONファイルから指定シーン所属のEditor管理Textを読み込み
