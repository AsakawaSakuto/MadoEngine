...
namespace MadoEngine::Beam {
	...
	public:
		...
		/// @brief Beam Effectの時間進行を一時停止
		void Pause();

		/// @brief Beam Effectの時間進行を再開
		void Resume();

		/// @brief Beam Effectの再生速度を設定
		/// @param playbackSpeed 設定する再生速度
		/// @return 有効な再生速度を設定できた場合はtrue
		bool SetPlaybackSpeed(float playbackSpeed);

		/// @brief Beam Effectが一時停止中か確認
		/// @return 一時停止中の場合はtrue
		bool IsPaused() const {
			return isPaused_;
		}

		/// @brief Instanceが終了済みか確認
		/// @return 終了済みの場合はtrue
		...
		/// @brief Beam描画データをRibbon Rendererへ登録
