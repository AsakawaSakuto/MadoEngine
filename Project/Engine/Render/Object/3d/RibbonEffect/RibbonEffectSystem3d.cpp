...
namespace {
	...
	constexpr std::size_t kMaximumRibbonAssetNameLength = 100;
	...
	bool IsValidAssetName(const std::string& assetName) {
		if (
			assetName.empty() ||
			assetName.size() > kMaximumRibbonAssetNameLength ||
			assetName.front() == ' ' ||
			assetName.back() == ' ' ||
			assetName.back() == '.') {
			return false;
...
namespace MadoEngine::Ribbon {
	...
	RibbonEffectSystem3d& RibbonEffectSystem3d::GetInstance() {
		static RibbonEffectSystem3d instance;
		return instance;
	}

	void RibbonEffectSystem3d::Initialize(
		ID3D12Device* device,
		...
		isInitialized_ = true;
		Logger::Output(
			"RibbonEffectSystem3dを初期化しました。Asset数: " + std::to_string(assetCount),
			Logger::Level::Engine
		);
	}

	void RibbonEffectSystem3d::Finalize() {
		renderer_.Finalize();
		assets_.clear();
		assetPaths_.clear();
		effectSlots_.clear();
	...
	std::size_t RibbonEffectSystem3d::LoadAssetsFromDirectory(
		const std::filesystem::path& directoryPath) {
		...
		if (error) {
			Logger::Output(
				"Ribbon Effect Asset Directoryを作成できません: " + PathToUtf8String(directoryPath),
				Logger::Level::Error
			);
			return 0;
		}
	...
	bool RibbonEffectSystem3d::LoadAsset(const std::filesystem::path& filePath) {
		auto asset = std::make_shared<RibbonEffectAsset>();
		...
		Logger::Output("Ribbon Effect Assetを登録しました: " + name, Logger::Level::Assets);
		return true;
	}

	bool RibbonEffectSystem3d::ReloadAsset(const std::string& assetName) {
	...
	bool RibbonEffectSystem3d::LoadAssetBackup(const std::string& assetName) {
		const auto found = assetPaths_.find(assetName);
		if (found == assetPaths_.end()) {
			return false;
		}
		...
		Logger::Output("Ribbon Effect AssetのBackupを読み込みました: " + assetName, Logger::Level::Assets);
		return true;
	}

	bool RibbonEffectSystem3d::CreateAsset(const std::string& assetName) {
	...
	bool RibbonEffectSystem3d::DuplicateAsset(
	...
	bool RibbonEffectSystem3d::RenameAsset(
		...
		RibbonEffectAsset renamed = *asset->second;
		renamed.SetName(newAssetName);
		renamed.Validate();
		if (!renamed.SaveToFile(newPath, false)) {
			return false;
		}
		auto loaded = std::make_shared<RibbonEffectAsset>();
		...
		Logger::Output(
			"Ribbon Effect Asset名を変更しました: " + assetName + " -> " + newAssetName,
			Logger::Level::Assets
		);
		return true;
	}

	bool RibbonEffectSystem3d::DeleteAsset(const std::string& assetName) {
		const auto asset = assets_.find(assetName);
		const auto path = assetPaths_.find(assetName);
		...
		assetPaths_.erase(path);
		Logger::Output(
			"Ribbon Effect AssetをTrashへ退避しました: " + PathToUtf8String(trashPath),
			Logger::Level::Assets
		);
		return true;
	}

	bool RibbonEffectSystem3d::IsAssetNameAvailable(const std::string& assetName) const {
	...
	RibbonEffectHandle RibbonEffectSystem3d::Play(
		const std::string& assetName,
		const RibbonEffectPlayDesc& desc) {
		if (!isInitialized_) {
			Logger::Output("初期化前にはRibbon Effectを再生できません。", Logger::Level::Warning);
			return {};
		}
		const auto found = assets_.find(assetName);
		if (found == assets_.end()) {
			Logger::Output("Ribbon Effect Assetが見つかりません: " + assetName, Logger::Level::Warning);
			return {};
		}
	...
	void RibbonEffectSystem3d::Stop(RibbonEffectHandle handle, RibbonStopMode mode) {
		if (RibbonEffectInstance* instance = Resolve(handle)) {
			instance->Stop(mode);
			isRenderDataPrepared_ = false;
		}
	}

	bool RibbonEffectSystem3d::SetTransform(
	...
	bool RibbonEffectSystem3d::SetControlPoints(
		...
		RibbonEffectInstance* instance = Resolve(handle);
		if (!instance || !instance->SetControlPoints(controlPoints)) {
			return false;
		}
		isRenderDataPrepared_ = false;
		return true;
	...
	bool RibbonEffectSystem3d::IsAlive(RibbonEffectHandle handle) const {
		return Resolve(handle) != nullptr;
	}

	void RibbonEffectSystem3d::Update(float deltaTime) {
		if (!isInitialized_) {
			return;
		}
		isRenderDataPrepared_ = false;
	...
	void RibbonEffectSystem3d::BeginFrame(uint64_t submissionFenceValue) {
		currentSubmissionFenceValue_ = submissionFenceValue;
		isRenderDataPrepared_ = false;
	}

	void RibbonEffectSystem3d::DrawLayerMask(
		SceneType sceneType,
		const Camera& camera,
	...
	void RibbonEffectSystem3d::OnGpuFrameCompleted(uint64_t completedFenceValue) {
		renderer_.OnGpuFrameCompleted(completedFenceValue);
	}

	void RibbonEffectSystem3d::ClearScene(SceneType sceneType) {
	...
	void RibbonEffectSystem3d::StopAll(RibbonStopMode mode) {
	...
	std::vector<std::string> RibbonEffectSystem3d::GetAssetNames() const {
	...
	const RibbonEffectAsset* RibbonEffectSystem3d::FindAsset(const std::string& assetName) const {
		const auto found = assets_.find(assetName);
		return found != assets_.end() ? found->second.get() : nullptr;
	}

	RibbonEffectAsset* RibbonEffectSystem3d::FindEditableAsset(const std::string& assetName) {
		const auto found = assets_.find(assetName);
		return found != assets_.end() ? found->second.get() : nullptr;
	}

	std::size_t RibbonEffectSystem3d::GetActiveEffectCount() const {
	...
	RibbonEffectInstance* RibbonEffectSystem3d::Resolve(RibbonEffectHandle handle) {
	...
	const RibbonEffectInstance* RibbonEffectSystem3d::Resolve(RibbonEffectHandle handle) const {
		if (!handle.HasValue() || handle.index >= effectSlots_.size()) {
			return nullptr;
		}
	...
	void RibbonEffectSystem3d::ReleaseSlot(uint32_t index) {
		EffectSlot& slot = effectSlots_[index];
		slot.instance.reset();
		++slot.generation;
