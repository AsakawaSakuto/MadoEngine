...
namespace MadoEngine::Particle {
	...
	public:
		...
		/// @brief Particle Systemを終了
		void Finalize();

		/// @brief 指定ディレクトリ内のParticle Assetを読み込み
		...
		/// @brief Particle Assetの名前とJsonファイル名を変更
		...
		/// @brief Particle Assetを登録解除してJsonファイルを退避
		...
		/// @brief 生存Particle総数を取得
		/// @return 生存Particle総数
		std::size_t GetAliveParticleCount() const;

		/// @brief Effect内EmitterのRuntime情報を取得
	...
	private:
		...
		/// @brief HandleからEffect Instanceを取得
