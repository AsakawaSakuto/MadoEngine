...
namespace MadoEngine::Render {
	...
	/// @brief フルスクリーンポストエフェクトの適用段階が有効か判定
	/// @param stage 判定する適用段階
	/// @return 有効な適用段階の場合はtrue
	...
	public:
		...
		/// @brief 個別描画レイヤー向けポストエフェクトの適用段階を設定
		/// @param stage 設定する適用段階
		void SetLayerEffectStage(LayerEffectStage stage);

		/// @brief 個別描画レイヤー向けポストエフェクトの適用段階を取得
		/// @return 設定されている適用段階
		LayerEffectStage GetLayerEffectStage() const;

		/// @brief 対象Layerを除外した描画Layerマスクを取得
		...
		/// @brief CPU側に保存しているパラメータサイズを取得
		/// @return パラメータサイズ
		std::size_t GetParameterDataSize() const;

		/// @brief ImGui編集用のfloatパラメータを追加
		...
		/// @brief ImGui表示名とは別の内部キーを持つfloatパラメータを追加
