...
namespace MadoEngine
{
	...
	public:
		...
		/// @brief Layer向けPostEffectPassを登録
		...
		/// @brief 画面全体に適用するポストエフェクトPassを登録
		...
		/// @brief 登録済みのLayer向けPostEffectPassをすべて削除
		void ClearLayerEffectPasses();

		/// @brief 登録済みの画面全体ポストエフェクトPassをすべて削除
		void ClearScreenEffectPasses();

		/// @brief 登録済みのLayer向けPostEffectPassの実行順Handle一覧を取得
		...
		/// @brief HandleからPassを描画処理中だけ使用する一時参照として取得
	...
	private:
		...
		/// @brief ウィンドウリサイズ要求を描画リソースへ反映
		void HandleResize();

		/// @brief 通常3D描画前にシャドウマップを生成してModelへ設定
		/// @param currentSceneType 現在のシーン種別
		/// @param shadowFocusPosition シャドウマップの中心へ置くワールド座標
		...
		/// @brief 指定したSRVを現在の描画先へポストエフェクト描画
		...
		/// @brief Scene段階のFog設定をParticle描画へ同期
		void UpdateParticleFogParameters();

		/// @brief シーンとLayerエフェクト結果を現在の描画先へ合成
		...
		/// @brief 次のLayerエフェクトチェーン出力先RenderTarget名を取得
