...
float SampleDissolveNoise(float2 texcoord, float noiseScale) {
    ...
    // UVを周期内へ折り返して拡大率変更時もNoise Texture外を参照しない制約
...
PixelShaderOutput main(VertexShaderOutput input) {
    ...
    // 進行境界の両側をSmoothstepで連続化して時間変化のちらつきを抑制
    ...
    // 元TextureのAlphaを維持しながら境界発光だけを可視領域へ追加
